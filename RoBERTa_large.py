...
PRE_TRAINED_MODEL_NAME = 'roberta-large'  # Changed to roberta-large
tokenizer = RobertaTokenizer.from_pretrained(PRE_TRAINED_MODEL_NAME)

MAX_LEN = 256  # Increased max length for roberta-large
...
class EssayScoringModel(nn.Module):
    def __init__(self, n_outputs, model_name):
        ...
        self.out = nn.Linear(self.roberta.config.hidden_size, n_outputs)
...
if __name__ == '__main__':
    ...
    EPOCHS = 10
    optimizer = AdamW(model.parameters(), lr=5e-6, correct_bias=False)  # Lower learning rate for RoBERTa-large
